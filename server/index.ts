...
const originalDb = new Pool({
  connectionString:
    "postgresql://neondb_owner:npg_ZH3VF9BEjlyk@ep-small-cloud-a46us4xp.us-east-1.aws.neon.tech/neondb?sslmode=require",
...
if (process.env.NODE_ENV === "production" && process.env.DATABASE_URL) {
  try {
    ...
    console.error(
      "[SESSION] PostgreSQL store failed, falling back to memory store:",
      error
    );
...
router.get("/tv-shows", async (req, res) => {
  try {
    ...
    if (req.query.stimulationScoreRange) {
      try {
        filters.stimulationScoreRange = JSON.parse(
          req.query.stimulationScoreRange as string
        );
      } catch (e) {
        return res
          .status(400)
          .json({ message: "Invalid stimulationScoreRange format" });
      }
    }
    ...
    // Sensory filters
    if (req.query.tantrumFactor)
      filters.tantrumFactor = req.query.tantrumFactor;
    if (req.query.interactionLevel)
      filters.interactionLevel = req.query.interactionLevel;
    if (req.query.dialogueIntensity)
      filters.dialogueIntensity = req.query.dialogueIntensity;
    if (req.query.soundFrequency)
      filters.soundFrequency = req.query.soundFrequency;
...
app.get("/media/tv-shows/:filename", async (req, res) => {
  try {
    ...
    const genericImagePath = join(
      __dirname,
      "../public/images/generic-tv-show.jpg"
    );
    ...
    cache: {
      memory: {
        ...
        hitRate:
          stats.stats.hits > 0
            ? (
                (stats.stats.hits / (stats.stats.hits + stats.stats.misses)) *
                100
              ).toFixed(2) + "%"
            : "0%",
...
if (process.env.NODE_ENV === "development") {
  ...
  try {
    ...
    console.log(
      "Static files not found, using development server for production deployment"
    );
    setupVite(app, server);
  }
}
...
process.on("uncaughtException", (error: any) => {
  ...
  if (
    ...
    console.log(
      "Database connection terminated - continuing operation with cache"
    );
